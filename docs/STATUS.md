...
tables, 53 RLS policies, 252 Vitest tests across 32 files, 63 pgTAP tests across 7 files, 9
...
| M4 | Storefront, SEO, conversion, motion | 8.5 | **WIP.** 96 items ticked, 24 open. Code is mostly ahead of Beco's content now: most of what remains is blocked on Drive folders, files and confirmations, not on build work. Full detail in `docs/milestones/M4-TODO.md` and `docs/milestones/M4-HANDOVER.md` |
...
Recorded through D71. D1 to roughly D45 are architectural, made before the build started, and
...
D66 through D71 are from this week specifically, including two real bugs, D67 and the splash
freeze under D66's own entry, found and fixed with a regression test proving each one.
